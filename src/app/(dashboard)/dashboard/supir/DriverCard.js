...
export default function DriverCard({ driver, onDelete }) {
    ...
    const imageUrl = driver.gambar_supir ? `${BASE_URL}${driver.gambar_supir}` : 'https://placehold.co/150x150';
    ...
    const handleDelete = async (id) => {
        try {
            ...
            if (response.ok) {
                ...
                // Instead of reloading, call the refresh function
                onDelete();
            } else {
                toast({
                    title: "Error",
    ...
    return <Card key={driver.id} className="hover:bg-muted transition-colors">
